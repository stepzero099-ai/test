...
import { Category, NewsArticle, UserPreferences } from './types';
...
export default function App() {
  ...
  const fetchNews = useCallback(async (isManualRefresh: boolean = false) => {
    ...
    try {
      ...
      setError('تعذر تحميل أحدث الأخبار. يرجى التحقق من اتصال الإنترنت والمحاولة مرة أخرى.');
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-['IBM_Plex_Sans_Arabic',sans-serif]">
      ...
      <BreakingNewsTicker
        ...
        onOpenArticle={(art) => setSelectedArticleForFullStory(art)}
      ...
      {/* 7. Signature "الحكاية من الأول" Modal */}
      <FullStoryModal
      ...
      {/* 11. In-App Push Notification Toast */}
      <NotificationToast
