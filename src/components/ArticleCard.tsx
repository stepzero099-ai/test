...
import { CATEGORIES_META, NewsArticle } from '../types';
...
export const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  return (
    <article
      ...
      className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col group"
    >
      {/* Top Media Thumbnail if available */}
      {article.imageUrl && (
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          {/* Metadata Row if no image */}
          {!article.imageUrl && (
            <div className="flex items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-1.5">
                <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${categoryMeta.bgLight}`}>
                  {categoryMeta.label}
                </span>

                {article.importance === 'breaking' && (
                  <span className="bg-red-600 text-white px-2 py-0.5 rounded-md text-xs font-bold flex items-center gap-1">
                    <Flame className="w-3.5 h-3.5 fill-white" />
                    عاجل
                  </span>
                )}
              ...
              {article.isCorroborated ? (
                <span className="flex items-center gap-1 text-emerald-600 text-xs font-semibold">
                  ...
                  مؤكد ({article.sourceCount} مصادر)
                </span>
              ) : (
